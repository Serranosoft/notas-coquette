...
export default function PitchVoiceSlider({ pitch, setVoicePitch, updateVoice }) {
    ...
    return (
        <View style={{ flex: 1 }}>
            <Slider
                ...
                upperLimit={1}
                value={pitch}
                onValueChange={(v) => setVoicePitch(Number(v.toFixed(2)))}
                onSlidingComplete={(v) => updateVoice({ pitch: Number(v.toFixed(2)) })}
