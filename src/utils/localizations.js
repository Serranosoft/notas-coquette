const translations = {
    es: {

        // Header Home
        _headerTitle: "Mis notas",
        _headerDropdownOption1: "Cambiar cuadricula",
        _headerDropdownOption2: "Configuración",

        // Header Note
        _headerNoteTitleNew: "Añadir nota",
        _headerNoteTitleEdit: "Editar nota",
        _headerNoteDropdownOption1: "Modo edición",
        _headerNoteDropdownOption2: "Modo lectura",
        _headerNoteDropdownOption3: "Eliminar nota",
        _headerNoteDropdownOption4: "Desbloquear nota",
        _headerNoteDropdownOption5: "Bloquear nota",
        _headerNoteDropdownOption6: "Configuración",

        // Home
        _homeButton: "Añadir nota",
        _homeEmptyList: "No tienes ninguna nota creada",

        // Lista de idiomas
        _langListSpanish: "Español",
        _langListEnglish: "Inglés",
        _langListArabic: "Árabe",
        _langListGerman: "Alemán",
        _langListFrench: "Francés",
        _langListHindi: "Hindu",
        _langListIndonesian: "Indonesio",
        _langListPortuguese: "Portugués",
        _langListRussian: "Ruso",
        _langListPolish: "Polaco",
        _langListVietnamese: "Vietnamita",
        _langListTurkish: "Turco",

        // Pantalla de settings
        _settingsApp: "Ajustes de la aplicación",
        _settingsLang: "Idioma de la aplicación",
        _settingsEditor: "Ajustes del editor",
        _settingsSave: "Guardado automático",
        _settingsTypo: "Tipografia",
        _settingsTypoWarning: "Al cambiar de tipografía se le enviará a la pantalla de inicio para cargar la nueva fuente",
        _settingsResetAll: "Reestablecer todo",
        _settingsRemoveAll: "Eliminar todas las notas",
        _settingsRemoveAllWarning: "¡CUIDADO! Al pulsar en el siguiente botón se eliminarán todas las notas guardadas",

        // 26-03-2025: Faltaban las siguientes variables por traducir
        _settingTitle: "Configuración",
        _noteInputPlaceholder: "Escribe tu nota...",
        _toastNoteSaved: "Nota guardada",
        _toastTypoSaved: "Tipografía guardada",
        _toastAutoSave: "Guardado automatico",
        _toastActivated: "Activado",
        _toastDeactivated: "Desactivado",
        _toastAllNotesDeleted: "Se han eliminado todas las notas",
        _lockNoteTitle: "Introduce el código PIN",
        _lockInfo: "Esta nota está bloqueada, introduce el código PIN para poder acceder a ella",
        _lockError: "PIN incorrecto",

        // 04-07-2025: Se agregan nuevas traducciones para nuevos textos
        _elementsSelected: "elementos seleccionados",
        _updatesModalNews: "Novedades",
        _updatesModalV1_1: "Añade Post-It en tus notas 📌",
        _updatesModalV1_2: "Incluye casillas de verificación ✔️",
        _updatesModalV1_3: "¡Ahora puedes tachar palabras! 📋",
        _updatesModalV1_4: "¡Ahora puedes justificar el texto! 📑",
        _updatesModalV1_5: "😊 Esperamos que disfrutes de todas estas novedades 😊",

    },
    en: {
        // Header Home
        _headerTitle: "My notes",
        _headerDropdownOption1: "Change grid",
        _headerDropdownOption2: "Settings",

        // Header Note
        _headerNoteTitleNew: "Add note",
        _headerNoteTitleEdit: "Edit note",
        _headerNoteDropdownOption1: "Edit mode",
        _headerNoteDropdownOption2: "Read mode",
        _headerNoteDropdownOption3: "Delete note",
        _headerNoteDropdownOption4: "Unlock note",
        _headerNoteDropdownOption5: "Lock note",
        _headerNoteDropdownOption6: "Settings",

        // Home
        _homeButton: "Add note",
        _homeEmptyList: "You don't have any notes created",

        // Lista de idiomas
        _langListSpanish: "Spanish",
        _langListEnglish: "English",
        _langListArabic: "Arabic",
        _langListGerman: "German",
        _langListFrench: "French",
        _langListHindi: "Hindi",
        _langListIndonesian: "Indonesian",
        _langListPortuguese: "Portuguese",
        _langListRussian: "Russian",
        _langListPolish: "Polish",
        _langListVietnamese: "Vietnamese",
        _langListTurkish: "Turkish",

        // Pantalla de settings
        _settingsApp: "Application settings",
        _settingsLang: "Application language",
        _settingsEditor: "Editor settings",
        _settingsSave: "Auto-save",
        _settingsTypo: "Typography",
        _settingsTypoWarning: "Changing the font will send you back to the home screen to load the new font",
        _settingsResetAll: "Reset all",
        _settingsRemoveAll: "Delete all notes",
        _settingsRemoveAllWarning: "WARNING! Pressing the button below will delete all saved notes",

        _settingTitle: "Settings",
        _noteInputPlaceholder: "Write your note...",
        _toastNoteSaved: "Note saved",
        _toastTypoSaved: "Typography saved",
        _toastAutoSave: "Auto-save",
        _toastActivated: "Activated",
        _toastDeactivated: "Deactivated",
        _toastAllNotesDeleted: "All notes have been deleted",
        _lockNoteTitle: "Enter the PIN code",
        _lockInfo: "This note is locked, enter the PIN code to access it",
        _lockError: "Incorrect PIN",

        _elementsSelected: "elements selected",
        _updatesModalNews: "News",
        _updatesModalV1_1: "Add Post-It to your notes 📌",
        _updatesModalV1_2: "Include checkboxes ✔️",
        _updatesModalV1_3: "Now you can strikethrough words! 📋",
        _updatesModalV1_4: "Now you can justify text! 📑",
        _updatesModalV1_5: "😊 We hope you enjoy all these new features 😊",

    },
    ar: {
        // Header Home
        _headerTitle: "ملاحظاتي",
        _headerDropdownOption1: "تغيير الشبكة",
        _headerDropdownOption2: "الإعدادات",

        // Header Note
        _headerNoteTitleNew: "إضافة ملاحظة",
        _headerNoteTitleEdit: "تعديل الملاحظة",
        _headerNoteDropdownOption1: "وضع التعديل",
        _headerNoteDropdownOption2: "وضع القراءة",
        _headerNoteDropdownOption3: "حذف الملاحظة",
        _headerNoteDropdownOption4: "فتح الملاحظة",
        _headerNoteDropdownOption5: "قفل الملاحظة",
        _headerNoteDropdownOption6: "الإعدادات",

        // Home
        _homeButton: "إضافة ملاحظة",
        _homeEmptyList: "ليس لديك أي ملاحظات",

        // Lista de idiomas
        _langListSpanish: "إسباني",
        _langListEnglish: "إنجليزي",
        _langListArabic: "عربي",
        _langListGerman: "ألماني",
        _langListFrench: "فرنسي",
        _langListHindi: "هندي",
        _langListIndonesian: "إندونيسي",
        _langListPortuguese: "برتغالي",
        _langListRussian: "روسي",
        _langListPolish: "بولندي",
        _langListVietnamese: "فيتنامي",
        _langListTurkish: "تركي",


        // Pantalla de settings
        _settingsApp: "إعدادات التطبيق",
        _settingsLang: "لغة التطبيق",
        _settingsEditor: "إعدادات المحرر",
        _settingsSave: "الحفظ التلقائي",
        _settingsTypo: "الخط",
        _settingsTypoWarning: "سيتم إعادتك إلى الشاشة الرئيسية لتحميل الخط الجديد عند تغيير الخط",
        _settingsResetAll: "إعادة ضبط كل شيء",
        _settingsRemoveAll: "حذف جميع الملاحظات",
        _settingsRemoveAllWarning: "تحذير! عند الضغط على الزر أدناه، سيتم حذف جميع الملاحظات المحفوظة",

        _settingTitle: "الإعدادات",
        _noteInputPlaceholder: "اكتب ملاحظتك...",
        _toastNoteSaved: "تم حفظ الملاحظة",
        _toastTypoSaved: "تم حفظ الخط",
        _toastAutoSave: "تم الحفظ التلقائي",
        _toastActivated: "مفعل",
        _toastDeactivated: "غير مفعل",
        _toastAllNotesDeleted: "تم حذف جميع الملاحظات",
        _lockNoteTitle: "أدخل رمز PIN",
        _lockInfo: "هذه الملاحظة مقفلة، أدخل رمز PIN للوصول إليها",
        _lockError: "رمز PIN غير صحيح",

        _elementsSelected: "عناصر محددة",
        _updatesModalNews: "أخبار",
        _updatesModalV1_1: "أضف الملاحظات اللاصقة إلى ملاحظاتك 📌",
        _updatesModalV1_2: "أدرج مربعات اختيار ✔️",
        _updatesModalV1_3: "الآن يمكنك شطب الكلمات! 📋",
        _updatesModalV1_4: "الآن يمكنك محاذاة النص! 📑",
        _updatesModalV1_5: "😊 نأمل أن تستمتع بكل هذه الميزات الجديدة 😊",

    },
    fr: {
        // Header Home
        _headerTitle: "Mes notes",
        _headerDropdownOption1: "Changer la grille",
        _headerDropdownOption2: "Paramètres",

        // Header Note
        _headerNoteTitleNew: "Ajouter une note",
        _headerNoteTitleEdit: "Modifier la note",
        _headerNoteDropdownOption1: "Mode édition",
        _headerNoteDropdownOption2: "Mode lecture",
        _headerNoteDropdownOption3: "Supprimer la note",
        _headerNoteDropdownOption4: "Déverrouiller la note",
        _headerNoteDropdownOption5: "Verrouiller la note",
        _headerNoteDropdownOption6: "Paramètres",

        // Home
        _homeButton: "Ajouter une note",
        _homeEmptyList: "Vous n'avez créé aucune note",

        // Lista de idiomas
        _langListSpanish: "Espagnol",
        _langListEnglish: "Anglais",
        _langListArabic: "Arabe",
        _langListGerman: "Allemand",
        _langListFrench: "Français",
        _langListHindi: "hindou",

        // Pantalla de settings
        _settingsApp: "Paramètres de l'application",
        _settingsLang: "Langue de l'application",
        _settingsEditor: "Paramètres de l'éditeur",
        _settingsSave: "Sauvegarde automatique",
        _settingsTypo: "Typographie",
        _settingsTypoWarning: "En changeant de police, vous serez redirigé vers l'écran d'accueil pour charger la nouvelle police",
        _settingsResetAll: "Réinitialiser tout",
        _settingsRemoveAll: "Supprimer toutes les notes",
        _settingsRemoveAllWarning: "ATTENTION ! En appuyant sur le bouton ci-dessous, toutes les notes enregistrées seront supprimées",

        _settingTitle: "Paramètres",
        _noteInputPlaceholder: "Écrivez votre note...",
        _toastNoteSaved: "Note enregistrée",
        _toastTypoSaved: "Typographie enregistrée",
        _toastAutoSave: "Sauvegarde automatique",
        _toastActivated: "Activé",
        _toastDeactivated: "Désactivé",
        _toastAllNotesDeleted: "Toutes les notes ont été supprimées",
        _lockNoteTitle: "Entrez le code PIN",
        _lockInfo: "Cette note est verrouillée, entrez le code PIN pour y accéder",
        _lockError: "Code PIN incorrect",

        _elementsSelected: "éléments sélectionnés",
        _updatesModalNews: "Nouveautés",
        _updatesModalV1_1: "Ajoutez des Post-It à vos notes 📌",
        _updatesModalV1_2: "Incluez des cases à cocher ✔️",
        _updatesModalV1_3: "Vous pouvez maintenant barrer des mots ! 📋",
        _updatesModalV1_4: "Vous pouvez maintenant justifier le texte ! 📑",
        _updatesModalV1_5: "😊 Nous espérons que tu apprécieras toutes ces nouveautés 😊",

    },
    de: {
        // Header Home
        _headerTitle: "Meine Notizen",
        _headerDropdownOption1: "Raster ändern",
        _headerDropdownOption2: "Einstellungen",

        // Header Note
        _headerNoteTitleNew: "Notiz hinzufügen",
        _headerNoteTitleEdit: "Notiz bearbeiten",
        _headerNoteDropdownOption1: "Bearbeitungsmodus",
        _headerNoteDropdownOption2: "Lesemodus",
        _headerNoteDropdownOption3: "Notiz löschen",
        _headerNoteDropdownOption4: "Notiz entsperren",
        _headerNoteDropdownOption5: "Notiz sperren",
        _headerNoteDropdownOption6: "Einstellungen",

        // Home
        _homeButton: "Notiz hinzufügen",
        _homeEmptyList: "Sie haben keine Notizen erstellt",

        // Lista de idiomas
        _langListSpanish: "Spanisch",
        _langListEnglish: "Englisch",
        _langListArabic: "Arabisch",
        _langListGerman: "Deutsch",
        _langListFrench: "Französisch",
        _langListHindi: "Hindi",
        _langListIndonesian: "Indonesisch",
        _langListPortuguese: "Portugiesisch",
        _langListRussian: "Russisch",
        _langListPolish: "Polnisch",
        _langListVietnamese: "Vietnamesisch",
        _langListTurkish: "Türkisch",

        // Pantalla de settings
        _settingsApp: "App-Einstellungen",
        _settingsLang: "App-Sprache",
        _settingsEditor: "Editor-Einstellungen",
        _settingsSave: "Automatisches Speichern",
        _settingsTypo: "Schriftart",
        _settingsTypoWarning: "Beim Ändern der Schriftart werden Sie zum Startbildschirm zurückgeleitet, um die neue Schrift zu laden",
        _settingsResetAll: "Alles zurücksetzen",
        _settingsRemoveAll: "Alle Notizen löschen",
        _settingsRemoveAllWarning: "WARNUNG! Durch Drücken der Schaltfläche unten werden alle gespeicherten Notizen gelöscht",

        _settingTitle: "Einstellungen",
        _noteInputPlaceholder: "Schreiben Sie Ihre Notiz...",
        _toastNoteSaved: "Notiz gespeichert",
        _toastTypoSaved: "Schriftart gespeichert",
        _toastAutoSave: "Automatisch gespeichert",
        _toastActivated: "Aktiviert",
        _toastDeactivated: "Deaktiviert",
        _toastAllNotesDeleted: "Alle Notizen wurden gelöscht",
        _lockNoteTitle: "Geben Sie den PIN-Code ein",
        _lockInfo: "Diese Notiz ist gesperrt, geben Sie den PIN-Code ein, um darauf zuzugreifen",
        _lockError: "Falscher PIN",

        _elementsSelected: "ausgewählte Elemente",
        _updatesModalNews: "Neuigkeiten",
        _updatesModalV1_1: "Füge Post-Its zu deinen Notizen hinzu 📌",
        _updatesModalV1_2: "Füge Kontrollkästchen hinzu ✔️",
        _updatesModalV1_3: "Jetzt kannst du Wörter durchstreichen! 📋",
        _updatesModalV1_4: "Jetzt kannst du den Text ausrichten! 📑",
        _updatesModalV1_5: "😊 Wir hoffen, du genießt all diese neuen Funktionen 😊",

    },
    hi: {
        // Header Home
        _headerTitle: "मेरे नोट्स",
        _headerDropdownOption1: "ग्रिड बदलें",
        _headerDropdownOption2: "सेटिंग्स",

        // Header Note
        _headerNoteTitleNew: "नोट जोड़ें",
        _headerNoteTitleEdit: "नोट संपादित करें",
        _headerNoteDropdownOption1: "संपादन मोड",
        _headerNoteDropdownOption2: "पढ़ने का मोड",
        _headerNoteDropdownOption3: "नोट हटाएं",
        _headerNoteDropdownOption4: "नोट अनलॉक करें",
        _headerNoteDropdownOption5: "नोट लॉक करें",
        _headerNoteDropdownOption6: "सेटिंग्स",

        // Home
        _homeButton: "नोट जोड़ें",
        _homeEmptyList: "आपने कोई नोट नहीं बनाया है",

        // Lista de idiomas
        _langListSpanish: "स्पेनिश",
        _langListEnglish: "अंग्रेजी",
        _langListArabic: "अरबी",
        _langListGerman: "जर्मन",
        _langListFrench: "फ्रेंच",
        _langListHindi: "हिंदी",
        _langListIndonesian: "इंडोनेशियाई",
        _langListPortuguese: "पुर्तगाली",
        _langListRussian: "रूसी",
        _langListPolish: "पोलिश",
        _langListVietnamese: "वियतनामी",
        _langListTurkish: "तुर्की",

        // Pantalla de settings
        _settingsApp: "एप्लिकेशन सेटिंग्स",
        _settingsLang: "एप्लिकेशन की भाषा",
        _settingsEditor: "संपादक सेटिंग्स",
        _settingsSave: "स्वचालित सहेजें",
        _settingsTypo: "टाइपोग्राफी",
        _settingsTypoWarning: "फ़ॉन्ट बदलने पर आपको नई फ़ॉन्ट लोड करने के लिए होम स्क्रीन पर वापस भेजा जाएगा",
        _settingsResetAll: "सब कुछ रीसेट करें",
        _settingsRemoveAll: "सभी नोट्स हटाएं",
        _settingsRemoveAllWarning: "चेतावनी! नीचे दिए गए बटन को दबाने से सभी सहेजे गए नोट्स हट जाएंगे",

        _settingTitle: "सेटिंग्स",
        _noteInputPlaceholder: "अपना नोट लिखें...",
        _toastNoteSaved: "नोट सहेजा गया",
        _toastTypoSaved: "टाइपोग्राफी सहेजी गई",
        _toastAutoSave: "स्वचालित सहेजा गया",
        _toastActivated: "सक्रिय",
        _toastDeactivated: "निष्क्रिय",
        _toastAllNotesDeleted: "सभी नोट हटा दिए गए हैं",
        _lockNoteTitle: "पिन कोड दर्ज करें",
        _lockInfo: "यह नोट लॉक है, इसे एक्सेस करने के लिए पिन कोड दर्ज करें",
        _lockError: "गलत पिन",

        _elementsSelected: "चयनित तत्व",
        _updatesModalNews: "समाचार",
        _updatesModalV1_1: "अपनी नोट्स में पोस्ट-इट जोड़ें 📌",
        _updatesModalV1_2: "चेकबॉक्स शामिल करें ✔️",
        _updatesModalV1_3: "अब आप शब्दों को काट सकते हैं! 📋",
        _updatesModalV1_4: "अब आप टेक्स्ट को जस्टिफाई कर सकते हैं! 📑",
        _updatesModalV1_5: "😊 हमें उम्मीद है कि आप इन सभी नए फीचर्स का आनंद लेंगे 😊",

    },
    id: {

        // Header Home
        _headerTitle: "Catatan saya",
        _headerDropdownOption1: "Ubah kisi",
        _headerDropdownOption2: "Pengaturan",

        // Header Note
        _headerNoteTitleNew: "Tambah catatan",
        _headerNoteTitleEdit: "Edit catatan",
        _headerNoteDropdownOption1: "Mode edit",
        _headerNoteDropdownOption2: "Mode baca",
        _headerNoteDropdownOption3: "Hapus catatan",
        _headerNoteDropdownOption4: "Buka kunci catatan",
        _headerNoteDropdownOption5: "Kunci catatan",
        _headerNoteDropdownOption6: "Pengaturan",

        // Home
        _homeButton: "Tambah catatan",
        _homeEmptyList: "Anda belum memiliki catatan",

        // Lista de idiomas
        _langListSpanish: "Spanyol",
        _langListEnglish: "Inggris",
        _langListArabic: "Arab",
        _langListGerman: "Jerman",
        _langListFrench: "Prancis",
        _langListHindi: "Hindi",
        _langListIndonesian: "Bahasa Indonesia",
        _langListPortuguese: "Bahasa Portugis",
        _langListRussian: "Bahasa Rusia",
        _langListPolish: "Bahasa Polandia",
        _langListVietnamese: "Tiếng Việt",
        _langListTurkish: "Türkçe",

        // Pantalla de settings
        _settingsApp: "Pengaturan aplikasi",
        _settingsLang: "Bahasa aplikasi",
        _settingsEditor: "Pengaturan editor",
        _settingsSave: "Simpan otomatis",
        _settingsTypo: "Tipografi",
        _settingsTypoWarning: "Saat mengubah tipografi, Anda akan diarahkan ke beranda untuk memuat font baru",
        _settingsResetAll: "Setel ulang semua",
        _settingsRemoveAll: "Hapus semua catatan",
        _settingsRemoveAllWarning: "PERINGATAN! Menekan tombol berikut akan menghapus semua catatan yang tersimpan",

        // 26-03-2025: Faltaban las siguientes variables por traducir
        _settingTitle: "Pengaturan",
        _noteInputPlaceholder: "Tulis catatan Anda...",
        _toastNoteSaved: "Catatan disimpan",
        _toastTypoSaved: "Tipografi disimpan",
        _toastAutoSave: "Simpan otomatis",
        _toastActivated: "Diaktifkan",
        _toastDeactivated: "Dinonaktifkan",
        _toastAllNotesDeleted: "Semua catatan telah dihapus",
        _lockNoteTitle: "Masukkan kode PIN",
        _lockInfo: "Catatan ini dikunci, masukkan kode PIN untuk membukanya",
        _lockError: "PIN salah",

        _elementsSelected: "elemen terpilih",
        _updatesModalNews: "Berita",
        _updatesModalV1_1: "Tambahkan Post-It ke catatanmu 📌",
        _updatesModalV1_2: "Sertakan kotak centang ✔️",
        _updatesModalV1_3: "Sekarang kamu bisa mencoret kata! 📋",
        _updatesModalV1_4: "Sekarang kamu bisa meratakan teks! 📑",
        _updatesModalV1_5: "😊 Semoga kamu menikmati semua fitur baru ini 😊",


    },
    pt: {

        // Header Home
        _headerTitle: "Minhas notas",
        _headerDropdownOption1: "Alterar grade",
        _headerDropdownOption2: "Configurações",

        // Header Note
        _headerNoteTitleNew: "Adicionar nota",
        _headerNoteTitleEdit: "Editar nota",
        _headerNoteDropdownOption1: "Modo de edição",
        _headerNoteDropdownOption2: "Modo de leitura",
        _headerNoteDropdownOption3: "Excluir nota",
        _headerNoteDropdownOption4: "Desbloquear nota",
        _headerNoteDropdownOption5: "Bloquear nota",
        _headerNoteDropdownOption6: "Configurações",

        // Home
        _homeButton: "Adicionar nota",
        _homeEmptyList: "Você ainda não tem nenhuma nota",

        // Lista de idiomas
        _langListSpanish: "Espanhol",
        _langListEnglish: "Inglês",
        _langListArabic: "Árabe",
        _langListGerman: "Alemão",
        _langListFrench: "Francês",
        _langListHindi: "Hindi",
        _langListIndonesian: "Indonésio",
        _langListPortuguese: "Português",
        _langListRussian: "Russo",
        _langListPolish: "Polonês",
        _langListVietnamese: "Vietnamita",
        _langListTurkish: "Turco",

        // Tela de configurações
        _settingsApp: "Configurações do aplicativo",
        _settingsLang: "Idioma do aplicativo",
        _settingsEditor: "Configurações do editor",
        _settingsSave: "Salvamento automático",
        _settingsTypo: "Tipografia",
        _settingsTypoWarning: "Ao alterar a tipografia, você será redirecionado para a tela inicial para carregar a nova fonte",
        _settingsResetAll: "Redefinir tudo",
        _settingsRemoveAll: "Excluir todas as notas",
        _settingsRemoveAllWarning: "CUIDADO! Ao clicar no botão a seguir, todas as notas salvas serão excluídas",

        // 26-03-2025: Variáveis que faltavam traduzir
        _settingTitle: "Configurações",
        _noteInputPlaceholder: "Escreva sua nota...",
        _toastNoteSaved: "Nota salva",
        _toastTypoSaved: "Tipografia salva",
        _toastAutoSave: "Salvamento automático",
        _toastActivated: "Ativado",
        _toastDeactivated: "Desativado",
        _toastAllNotesDeleted: "Todas as notas foram excluídas",
        _lockNoteTitle: "Digite o código PIN",
        _lockInfo: "Esta nota está bloqueada, digite o código PIN para acessá-la",
        _lockError: "PIN incorreto",

        _elementsSelected: "elementos selecionados",
        _updatesModalNews: "Novidades",
        _updatesModalV1_1: "Adicione Post-It às suas notas 📌",
        _updatesModalV1_2: "Inclua caixas de seleção ✔️",
        _updatesModalV1_3: "Agora você pode riscar palavras! 📋",
        _updatesModalV1_4: "Agora você pode justificar o texto! 📑",
        _updatesModalV1_5: "😊 Esperamos que você aproveite todos esses novos recursos 😊",


    },
    ru: {

        // Header Home
        _headerTitle: "Мои заметки",
        _headerDropdownOption1: "Изменить сетку",
        _headerDropdownOption2: "Настройки",

        // Header Note
        _headerNoteTitleNew: "Добавить заметку",
        _headerNoteTitleEdit: "Редактировать заметку",
        _headerNoteDropdownOption1: "Режим редактирования",
        _headerNoteDropdownOption2: "Режим чтения",
        _headerNoteDropdownOption3: "Удалить заметку",
        _headerNoteDropdownOption4: "Разблокировать заметку",
        _headerNoteDropdownOption5: "Заблокировать заметку",
        _headerNoteDropdownOption6: "Настройки",

        // Home
        _homeButton: "Добавить заметку",
        _homeEmptyList: "У вас нет созданных заметок",

        // Список языков
        _langListSpanish: "Испанский",
        _langListEnglish: "Английский",
        _langListArabic: "Арабский",
        _langListGerman: "Немецкий",
        _langListFrench: "Французский",
        _langListHindi: "Хинди",
        _langListIndonesian: "Индонезийский",
        _langListPortuguese: "Португальский",
        _langListRussian: "Русский",
        _langListPolish: "Польский",
        _langListVietnamese: "Вьетнамский",
        _langListTurkish: "Турецкий",

        // Экран настроек
        _settingsApp: "Настройки приложения",
        _settingsLang: "Язык приложения",
        _settingsEditor: "Настройки редактора",
        _settingsSave: "Автосохранение",
        _settingsTypo: "Шрифт",
        _settingsTypoWarning: "При смене шрифта вы будете перенаправлены на главный экран для загрузки нового шрифта",
        _settingsResetAll: "Сбросить всё",
        _settingsRemoveAll: "Удалить все заметки",
        _settingsRemoveAllWarning: "ВНИМАНИЕ! Нажав следующую кнопку, вы удалите все сохранённые заметки",

        // 26-03-2025: Перевод недостающих переменных
        _settingTitle: "Настройки",
        _noteInputPlaceholder: "Напишите свою заметку...",
        _toastNoteSaved: "Заметка сохранена",
        _toastTypoSaved: "Шрифт сохранён",
        _toastAutoSave: "Автосохранение",
        _toastActivated: "Активировано",
        _toastDeactivated: "Отключено",
        _toastAllNotesDeleted: "Все заметки удалены",
        _lockNoteTitle: "Введите PIN-код",
        _lockInfo: "Эта заметка заблокирована. Введите PIN-код для доступа",
        _lockError: "Неверный PIN-код",

        _elementsSelected: "выбранные элементы",
        _updatesModalNews: "Новости",
        _updatesModalV1_1: "Добавляйте стикеры в свои заметки 📌",
        _updatesModalV1_2: "Вставляйте флажки ✔️",
        _updatesModalV1_3: "Теперь можно зачёркивать слова! 📋",
        _updatesModalV1_4: "Теперь можно выравнивать текст! 📑",
        _updatesModalV1_5: "😊 Надеемся, вам понравятся все эти новшества 😊",


    },
    pl: {

        // Header Home
        _headerTitle: "Moje notatki",
        _headerDropdownOption1: "Zmień siatkę",
        _headerDropdownOption2: "Ustawienia",

        // Header Note
        _headerNoteTitleNew: "Dodaj notatkę",
        _headerNoteTitleEdit: "Edytuj notatkę",
        _headerNoteDropdownOption1: "Tryb edycji",
        _headerNoteDropdownOption2: "Tryb czytania",
        _headerNoteDropdownOption3: "Usuń notatkę",
        _headerNoteDropdownOption4: "Odblokuj notatkę",
        _headerNoteDropdownOption5: "Zablokuj notatkę",
        _headerNoteDropdownOption6: "Ustawienia",

        // Home
        _homeButton: "Dodaj notatkę",
        _homeEmptyList: "Nie masz jeszcze żadnych notatek",

        // Lista języków
        _langListSpanish: "Hiszpański",
        _langListEnglish: "Angielski",
        _langListArabic: "Arabski",
        _langListGerman: "Niemiecki",
        _langListFrench: "Francuski",
        _langListHindi: "Hinduski",
        _langListIndonesian: "Indonezyjski",
        _langListPortuguese: "Portugalski",
        _langListRussian: "Rosyjski",
        _langListPolish: "Polski",
        _langListVietnamese: "Wietnamski",
        _langListTurkish: "Turecki",

        // Ekran ustawień
        _settingsApp: "Ustawienia aplikacji",
        _settingsLang: "Język aplikacji",
        _settingsEditor: "Ustawienia edytora",
        _settingsSave: "Automatyczne zapisywanie",
        _settingsTypo: "Typografia",
        _settingsTypoWarning: "Po zmianie typografii zostaniesz przekierowany na ekran główny, aby załadować nową czcionkę",
        _settingsResetAll: "Zresetuj wszystko",
        _settingsRemoveAll: "Usuń wszystkie notatki",
        _settingsRemoveAllWarning: "UWAGA! Naciśnięcie poniższego przycisku spowoduje usunięcie wszystkich zapisanych notatek",

        // 26-03-2025: Brakujące tłumaczenia
        _settingTitle: "Ustawienia",
        _noteInputPlaceholder: "Napisz swoją notatkę...",
        _toastNoteSaved: "Notatka zapisana",
        _toastTypoSaved: "Typografia zapisana",
        _toastAutoSave: "Automatyczne zapisywanie",
        _toastActivated: "Aktywowane",
        _toastDeactivated: "Dezaktywowane",
        _toastAllNotesDeleted: "Wszystkie notatki zostały usunięte",
        _lockNoteTitle: "Wprowadź kod PIN",
        _lockInfo: "Ta notatka jest zablokowana. Wprowadź kod PIN, aby uzyskać dostęp",
        _lockError: "Nieprawidłowy kod PIN",

        _elementsSelected: "wybrane elementy",
        _updatesModalNews: "Nowości",
        _updatesModalV1_1: "Dodaj karteczki Post-It do swoich notatek 📌",
        _updatesModalV1_2: "Dodaj pola wyboru ✔️",
        _updatesModalV1_3: "Teraz możesz przekreślać słowa! 📋",
        _updatesModalV1_4: "Teraz możesz justować tekst! 📑",
        _updatesModalV1_5: "😊 Mamy nadzieję, że spodoba Ci się wszystkie te nowości 😊",


    },
    vi: {

        // Header Home
        _headerTitle: "Ghi chú của tôi",
        _headerDropdownOption1: "Thay đổi lưới",
        _headerDropdownOption2: "Cài đặt",

        // Header Note
        _headerNoteTitleNew: "Thêm ghi chú",
        _headerNoteTitleEdit: "Chỉnh sửa ghi chú",
        _headerNoteDropdownOption1: "Chế độ chỉnh sửa",
        _headerNoteDropdownOption2: "Chế độ đọc",
        _headerNoteDropdownOption3: "Xoá ghi chú",
        _headerNoteDropdownOption4: "Mở khóa ghi chú",
        _headerNoteDropdownOption5: "Khóa ghi chú",
        _headerNoteDropdownOption6: "Cài đặt",

        // Home
        _homeButton: "Thêm ghi chú",
        _homeEmptyList: "Bạn chưa có ghi chú nào",

        // Danh sách ngôn ngữ
        _langListSpanish: "Tiếng Tây Ban Nha",
        _langListEnglish: "Tiếng Anh",
        _langListArabic: "Tiếng Ả Rập",
        _langListGerman: "Tiếng Đức",
        _langListFrench: "Tiếng Pháp",
        _langListHindi: "Tiếng Hindi",
        _langListIndonesian: "Tiếng Indonesia",
        _langListPortuguese: "Tiếng Bồ Đào Nha",
        _langListRussian: "Tiếng Nga",
        _langListPolish: "Tiếng Ba Lan",
        _langListVietnamese: "Tiếng Việt",
        _langListTurkish: "Tiếng Thổ Nhĩ Kỳ",

        // Màn hình cài đặt
        _settingsApp: "Cài đặt ứng dụng",
        _settingsLang: "Ngôn ngữ ứng dụng",
        _settingsEditor: "Cài đặt trình chỉnh sửa",
        _settingsSave: "Tự động lưu",
        _settingsTypo: "Phông chữ",
        _settingsTypoWarning: "Khi thay đổi phông chữ, bạn sẽ được chuyển về màn hình chính để tải phông mới",
        _settingsResetAll: "Đặt lại tất cả",
        _settingsRemoveAll: "Xoá tất cả ghi chú",
        _settingsRemoveAllWarning: "CẢNH BÁO! Nhấn nút dưới đây sẽ xoá toàn bộ ghi chú đã lưu",

        // 26-03-2025: Biến còn thiếu
        _settingTitle: "Cài đặt",
        _noteInputPlaceholder: "Viết ghi chú của bạn...",
        _toastNoteSaved: "Đã lưu ghi chú",
        _toastTypoSaved: "Đã lưu phông chữ",
        _toastAutoSave: "Tự động lưu",
        _toastActivated: "Đã bật",
        _toastDeactivated: "Đã tắt",
        _toastAllNotesDeleted: "Đã xoá tất cả ghi chú",
        _lockNoteTitle: "Nhập mã PIN",
        _lockInfo: "Ghi chú này đã bị khóa, hãy nhập mã PIN để truy cập",
        _lockError: "Sai mã PIN",

        _elementsSelected: "các phần tử đã chọn",
        _updatesModalNews: "Tin tức",
        _updatesModalV1_1: "Thêm Post-It vào ghi chú của bạn 📌",
        _updatesModalV1_2: "Bao gồm hộp kiểm ✔️",
        _updatesModalV1_3: "Bây giờ bạn có thể gạch ngang chữ! 📋",
        _updatesModalV1_4: "Bây giờ bạn có thể căn đều văn bản! 📑",
        _updatesModalV1_5: "😊 Hy vọng bạn sẽ thích tất cả những tính năng mới này 😊",


    },
    tr: {

        // Header Home
        _headerTitle: "Notlarım",
        _headerDropdownOption1: "Izgarayı değiştir",
        _headerDropdownOption2: "Ayarlar",

        // Header Note
        _headerNoteTitleNew: "Not ekle",
        _headerNoteTitleEdit: "Notu düzenle",
        _headerNoteDropdownOption1: "Düzenleme modu",
        _headerNoteDropdownOption2: "Okuma modu",
        _headerNoteDropdownOption3: "Notu sil",
        _headerNoteDropdownOption4: "Notun kilidini aç",
        _headerNoteDropdownOption5: "Notu kilitle",
        _headerNoteDropdownOption6: "Ayarlar",

        // Home
        _homeButton: "Not ekle",
        _homeEmptyList: "Henüz oluşturulmuş bir notunuz yok",

        // Dil listesi
        _langListSpanish: "İspanyolca",
        _langListEnglish: "İngilizce",
        _langListArabic: "Arapça",
        _langListGerman: "Almanca",
        _langListFrench: "Fransızca",
        _langListHindi: "Hintçe",
        _langListIndonesian: "Endonezce",
        _langListPortuguese: "Portekizce",
        _langListRussian: "Rusça",
        _langListPolish: "Lehçe",
        _langListVietnamese: "Vietnamca",
        _langListTurkish: "Türkçe",

        // Ayarlar ekranı
        _settingsApp: "Uygulama ayarları",
        _settingsLang: "Uygulama dili",
        _settingsEditor: "Editör ayarları",
        _settingsSave: "Otomatik kaydet",
        _settingsTypo: "Yazı tipi",
        _settingsTypoWarning: "Yazı tipi değiştirildiğinde yeni fontu yüklemek için ana ekrana yönlendirileceksiniz",
        _settingsResetAll: "Tümünü sıfırla",
        _settingsRemoveAll: "Tüm notları sil",
        _settingsRemoveAllWarning: "DİKKAT! Aşağıdaki butona bastığınızda tüm kaydedilmiş notlar silinecektir",

        // 26-03-2025: Eksik çeviriler
        _settingTitle: "Ayarlar",
        _noteInputPlaceholder: "Notunuzu yazın...",
        _toastNoteSaved: "Not kaydedildi",
        _toastTypoSaved: "Yazı tipi kaydedildi",
        _toastAutoSave: "Otomatik kaydetme",
        _toastActivated: "Etkinleştirildi",
        _toastDeactivated: "Devre dışı bırakıldı",
        _toastAllNotesDeleted: "Tüm notlar silindi",
        _lockNoteTitle: "PIN kodunu girin",
        _lockInfo: "Bu not kilitli, erişmek için PIN kodunu girin",
        _lockError: "Hatalı PIN",

        _elementsSelected: "seçilen öğeler",
        _updatesModalNews: "Yenilikler",
        _updatesModalV1_1: "Notlarınıza Post-It ekleyin 📌",
        _updatesModalV1_2: "Onay kutuları ekleyin ✔️",
        _updatesModalV1_3: "Artık kelimeleri üstünü çizebilirsiniz! 📋",
        _updatesModalV1_4: "Artık metni hizalayabilirsiniz! 📑",
        _updatesModalV1_5: "😊 Umarız bu yeni özelliklerin keyfini çıkarırsın 😊",


    },



}

export { translations };